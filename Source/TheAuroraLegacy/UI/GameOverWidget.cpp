    ...
    FName CurrentLevelName)
{
    ...
    if (LevelText)
    {
        FString Text = FString::Printf(
            TEXT("Caíste en el Nivel %d"), LevelWhenDied);
        LevelText->SetText(FText::FromString(Text));
    }

    if (GameOverMessage)
    {
        GameOverMessage->SetText(FText::FromString(
            GetMessageForPhase(PhaseWhenDied)));
    }
}
...
void UGameOverWidget::OnRetryClicked()
{
    // Reintentar desde el nivel donde murió — el GameInstance
    // conserva el score y las vidas actuales (no se resetea)
    APlayerController* PC = GetWorld()->GetFirstPlayerController();
    if (PC)
    {
        PC->SetShowMouseCursor(false);
...
void UGameOverWidget::OnRestartClicked()
{
    ...
    if (UAuroraGameInstance* GI = Cast<UAuroraGameInstance>(
        GetGameInstance()))
    {
        GI->ResetStats(); // score=0, lives=3, currentLevel=1
    }
...
void UGameOverWidget::OnMainMenuClicked()
{
    // Al volver al menú no reseteamos — el jugador puede
    // volver a jugar desde cero usando el botón Play del menú,
    // que sí llama ResetStats()
